...
VALID_TRANSACTION_DEBIT = dict()
VALID_TRANSACTION_DEBIT["amount"] = "100"
VALID_TRANSACTION_DEBIT["transactionType"] = "debit"
...
def test_post_transaction_no_balance_credit(client):
    headers = {"content-type": "application/json"}
    post_response = client.post(
        "/test/api/transaction/{}".format(EXAMPLE_USER_ID_NO_BALANCE),
        data=json.dumps(VALID_TRANSACTION),
        headers=headers,
    )
    print(post_response.get_json())
...
def test_post_transaction_no_balance_debit(client):
    headers = {"content-type": "application/json"}
    post_response = client.post(
        "/test/api/transaction/{}".format(EXAMPLE_USER_ID_NO_BALANCE),
        data=json.dumps(VALID_TRANSACTION_DEBIT),
        headers=headers,
    )
    print(post_response.get_json())
